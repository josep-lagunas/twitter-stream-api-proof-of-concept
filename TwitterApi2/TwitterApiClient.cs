...
namespace TwitterApi
{
    ...
    public class TwitterApiClient : ITwitterApiClient
    {
        ...
        public bool StopStreamingTweetsByHashTags(string id)
        {
            ...
            CancellationTokenSource cancelationTokenSrc;
            var removed = streamingCancellationTokensSrc.TryRemove(id, out cancelationTokenSrc);
            
            if (streamingCancellationTokensSrc != null && removed)
            {
            ...
            StreamTweetByHashtagHandler streamTweetHandler)
        {
            ...
            if ((hashtags == null || !hashtags.Any())
                ...
                && (mapBoxCoordinates == null || mapBoxCoordinates.Any()))
            {
            ...
            StreamTweetByHashtagHandler streamTweetHandler)
        {
            ...
            httpInvoker.HttpPostStreamInvoke(url, headers, contentHeaders,
                ...
                (target, e) =>
                {
                    var json = e.Result.HttpContent;
                    Tweet tweet;
                    if (SerializationServices.GetInstance.TryParse<Tweet>(json, out tweet))
                    {
        ...
        private async void ConsumeStreamAPIDoWorkOld(string url, Dictionary<string, string> data)
        {
            ...
            // Build the OAuth HTTP Header from the data.
            var oAuthHeader = GenerateOAuthHeader(data, url);
            ...
            using (var streamContent = new StreamContent(new MemoryStream(postParameters)))
            {
        ...
        private static string GenerateOAuthHeader(Dictionary<string, string> data, string url)
        {
            var result = "OAuth " + string.Join(
                                ", ",
                                data
